...
func Parse(conf string) error {
	...
	/*
		    ...
		    config:
		      ...
		      querydatachs: 100
		      callTimeout: 1000
		      debugprt: 2 #if 0 not print, if 1 print the sql,
		      taglen: 128
		      taglimit: 1024
		      tagnumlimit: 128
		      ...
		      tagstr: ""
	*/
	viper.SetDefault("backend.taosd", map[string]interface{}{
		"enabled": true,
		"name":    "taosd",
	...
	if Config.Backend.TDengine.Enabled {
		// viper.Unmarshal not compatible with yaml.Unmarshal

		var taosconfig taosd.TDengineSection
		err := taosd.YamlParse([]byte(bs), &taosconfig)
		if err != nil {
			return err
		}
		Config.Backend.TDengine = taosconfig
	}
