...
func (query *ShowSeries) renderTimeRange() {
	// time
	if strings.Contains(query.RawQuery, "WHERE") {
		query.RawQuery = fmt.Sprintf("%s AND ts >= %d AND ts <= %d", query.RawQuery,
			query.Start*1000, //time.Duration(query.Start)*time.Second,
			query.End*1000)   //time.Duration(query.End)*time.Second)
	} else {
		query.RawQuery = fmt.Sprintf("%s WHERE ts >= %d AND ts <= %d", query.RawQuery,
			query.Start*1000, //time.Duration(query.Start)*time.Second,
			query.End*1000)   //time.Duration(query.End)*time.Second)
	}
}
...
func (query *QueryData) renderSelect() {
	// select
	stabname := MetricnameEscape(query.Metric)
	if query.AggrFunc != "" && len(query.GroupKey) > 0 {
		query.RawQuery = ""
	} else {
		query.RawQuery = fmt.Sprintf("SELECT ts,value FROM %s ", stabname)
	}
}
...
func (query *QueryData) renderTimeRange() {
	...
	if strings.Contains(query.RawQuery, "WHERE") {
		query.RawQuery = fmt.Sprintf("%s AND ts >= %d AND ts <= %d ", query.RawQuery,
			query.Start*1000, //time.Duration(query.Start)*time.Second/1000,
			query.End*1000)   //time.Duration(query.End)*time.Second/1000)
	} else {
		query.RawQuery = fmt.Sprintf("%s WHERE ts >= %d AND ts <= %d ", query.RawQuery, query.Start, query.End)
	}
}
